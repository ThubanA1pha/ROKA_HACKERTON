...
```
Firestore
 ├─ weapons            화기 마스터 (K1 / K2 / K2C1 / M16A1, 4개 문서 고정)
...
- 현재 YOLO 모델은 `m16`, `k2` 2종만 학습되어 있다(팀원 쪽 `data/data_combined.yaml` 기준). `weapons` 컬렉션은 4종 모두 미리 등록해 두었으니, K1/K2C1 학습 데이터가 추가되는 대로 모델만 갱신하면 된다.
- `detectionRecords`는 아직 `src/app.py`와 통합되지 않았다. 통합 시 사용자가 오탐을 제거/확정하는 단계를 거친 뒤 `save_detection_record()`를 호출하도록 연결해야 한다.
